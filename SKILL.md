...
**Runtime:** Run bundled Python tools through `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run <script.py> [args]`. This is a thin dispatcher, not an isolated Python environment: it runs scripts with whatever `python3` is already on PATH. Install dependencies once with `pip install -r requirements.txt` from the skill's root. Never invoke bundled scripts with a bare Python interpreter; the launcher resolves paths correctly regardless of the caller's working directory, a bare `python3 scripts/foo.py` does not.
