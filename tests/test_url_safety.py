...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason="os.chmod on Windows only toggles a read-only flag, it does not "
    "implement POSIX permission bits, so 0o600/0o644 assertions are "
    "meaningless here. This protection is real and tested on POSIX.",
)
...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason="POSIX permission bits are not meaningful on Windows, see "
    "test_save_oauth_token_writes_0o600 for the full explanation.",
)
...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason="os.fchmod does not exist on Windows at all.",
)
