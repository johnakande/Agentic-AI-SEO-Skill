...
- Raw HTML + rendered HTML (via `python3 <SKILL_DIR>/scripts/render_page.py`)
...
- Core Web Vitals field data from CrUX (via `python3 <SKILL_DIR>/scripts/pagespeed_check.py`)
...
- A PDF via `python3 <SKILL_DIR>/scripts/google_report.py` when the user asks for one.
