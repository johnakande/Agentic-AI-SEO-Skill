...
- Raw HTML + rendered HTML (via `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run render_page.py`)
...
- Core Web Vitals field data from CrUX (via `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run pagespeed_check.py`)
...
- A PDF via `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run google_report.py` when the user asks for one.
