...
2. **Moz API** (free signup): `python3 <SKILL_DIR>/scripts/backlinks_auth.py --check moz --json`
3. **Bing Webmaster** (free signup): `python3 <SKILL_DIR>/scripts/backlinks_auth.py --check bing --json`
...
Run `python3 <SKILL_DIR>/scripts/backlinks_auth.py --check --json` to detect all sources at once.
...
**Moz API:** `python3 <SKILL_DIR>/scripts/moz_api.py metrics <url> --json` → Domain Authority, Page Authority, Spam Score, linking root domains, external links.

**Common Crawl:** `python3 <SKILL_DIR>/scripts/commoncrawl_graph.py <domain> --json` → PageRank, harmonic centrality, and low-confidence rank/presence data.
...
**Moz API:** `python3 <SKILL_DIR>/scripts/moz_api.py anchors <url> --json`

**Bing Webmaster:** `python3 <SKILL_DIR>/scripts/bing_webmaster.py links <url> --json` (extract anchor text from link details)
...
**Moz API:** `python3 <SKILL_DIR>/scripts/moz_api.py domains <url> --json` → domains with DA scores

**Common Crawl:** `python3 <SKILL_DIR>/scripts/commoncrawl_graph.py <domain> --json` → domain-level rank/presence data, no verified referring-domain counts
...
**Moz API:** Raw vendor spam_score from `python3 <SKILL_DIR>/scripts/moz_api.py metrics <url> --json` (source-label the value; apply thresholds only if verified against current Moz docs)

**Verification Crawler:** `python3 <SKILL_DIR>/scripts/verify_backlinks.py --target <url> --links <file> --json` (verify suspicious links still exist)
...
**Moz API:** `python3 <SKILL_DIR>/scripts/moz_api.py pages <domain> --json`
...
**Bing Webmaster:** `python3 <SKILL_DIR>/scripts/bing_webmaster.py compare <url1> <url2> --json`
...
**Moz API:** Compare DA/PA between domains via `python3 <SKILL_DIR>/scripts/moz_api.py metrics <url> --json` for each
...
**Verification Crawler:** For known links, verify current status with `python3 <SKILL_DIR>/scripts/verify_backlinks.py --target <url> --links <file> --json`
