...
2. **Moz API** (free signup): `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run backlinks_auth.py --check moz --json`
3. **Bing Webmaster** (free signup): `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run backlinks_auth.py --check bing --json`
...
Run `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run backlinks_auth.py --check --json` to detect all sources at once.
...
**Moz API:** `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run moz_api.py metrics <url> --json` → Domain Authority, Page Authority, Spam Score, linking root domains, external links.

**Common Crawl:** `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run commoncrawl_graph.py <domain> --json` → PageRank, harmonic centrality, and low-confidence rank/presence data.
...
**Moz API:** `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run moz_api.py anchors <url> --json`

**Bing Webmaster:** `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run bing_webmaster.py links <url> --json` (extract anchor text from link details)
...
**Moz API:** `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run moz_api.py domains <url> --json` → domains with DA scores

**Common Crawl:** `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run commoncrawl_graph.py <domain> --json` → domain-level rank/presence data, no verified referring-domain counts
...
**Moz API:** Raw vendor spam_score from `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run moz_api.py metrics <url> --json` (source-label the value; apply thresholds only if verified against current Moz docs)

**Verification Crawler:** `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run verify_backlinks.py --target <url> --links <file> --json` (verify suspicious links still exist)
...
**Moz API:** `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run moz_api.py pages <domain> --json`
...
**Bing Webmaster:** `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run bing_webmaster.py compare <url1> <url2> --json`
...
**Moz API:** Compare DA/PA between domains via `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run moz_api.py metrics <url> --json` for each
...
**Verification Crawler:** For known links, verify current status with `"$HOME/.claude/skills/seo-skill/bin/seo-skill" run verify_backlinks.py --target <url> --links <file> --json`
