...
### On `/seo flow sync`
Not available in this build. The 41 prompts here are bundled as a static
snapshot rather than synced live from the upstream FLOW repo via `gh`. To
refresh them, re-pull `references/` from `github.com/AgriciDaniel/flow` by
hand.
...
| Scenario | Action |
|----------|--------|
| `references/flow-framework.md` missing | The bundled snapshot is incomplete or was partially installed; re-copy `resources/skills/seo-flow/references/` from the repo. |
| Prompt file missing | Same as above, a specific prompt file didn't make it into the local copy. |
